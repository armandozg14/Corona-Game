        ...
        for(int j = 0; j < M; j++)
        {
            ...
            switch (tile)
            {
                ...
                case 'e':
                    ...
                    break;
                case 'v':
                    container->push_back(new Tile(cropSize * 2, cropSize * 5, scale, cropSize, texture, world));
                    container->back()->TurnPhysicsOn(cropSize * scale * j, cropSize * scale * i);
                    break;
                case 'n':
                    container->push_back(new Tile(cropSize * 3, cropSize * 4, scale, cropSize, texture, world));
                    container->back()->TurnPhysicsOn(cropSize * scale * j, cropSize * scale * i);
                    break;
                case 'g':
